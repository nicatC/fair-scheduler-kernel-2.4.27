...
int schedMode = 0; //PHASE2
EXPORT_SYMBOL(schedMode);
/*
 * scheduler variables
 */
...
//PHASE2
	if(schedMode == 0){
		...
		printk(KERN_INFO "Fair-Share Scheduler is now active (schedMode = 1).\n");
        
		int activeUser = 0;
		int user_count[10] = {0}; //number of process for each user
		int user_processes[10] = {0};

        // Iterate over the runqueue
        list_for_each(tmp, &runqueue_head) {
            p = list_entry(tmp, struct task_struct, run_list);
            if (can_schedule(p, this_cpu)) {
				if(p->uid >= 1000 && p->uid <= 1009)
                user_count[(p->uid) - 1000]++; // count number of process for each user
				user_processes[(p->uid) - 1000] = 1;
            }
        }
		int i;
		for( i=0; i<10; i++){
			if(user_processes[i] == 1){
				activeUser++;
			}
		}
		float user_share_f = 0.0f;
		if (activeUser > 0)
			user_share_f = 100.0f / (float)activeUser;
        next = idle_task(this_cpu);
		c=-1000;
		...
		list_for_each(tmp, &runqueue_head) {
			p = list_entry(tmp, struct task_struct, run_list);
			if (can_schedule(p, this_cpu)) {
				if (p->uid >= 1000 && p->uid <= 1009 && activeUser > 0) {
					if (user_count[p->uid - 1000] > 0) {
						float process_share_f = user_share_f / (float)user_count[p->uid - 1000];
						float dynamic_weight_f = (float)goodness(p, this_cpu, prev->active_mm) * process_share_f;
						int weight = (int)(dynamic_weight_f);
